...
if os.path.isfile(recfile):
    ...
    try:
        last_URI = rec_URIs[-1]
        print "last URI and last datatype are: {}".format(last_URI)
...
for URI in datasets:
    ...
    print "\nsubmitting FeatureWeightedGridStatistics requests for each DataType..."
    ...
    for d in datatypes_list:
        ...
        while not done:
            try:
                ...
                new_ofname = os.path.join(outpath, d+'_'+URI[-5:]+'.csv')
                ofp.write(outfile + ',' + new_ofname + '\n')
                shutil.move(outfile, new_ofname)
                print "saved as: {}".format(new_ofname)
