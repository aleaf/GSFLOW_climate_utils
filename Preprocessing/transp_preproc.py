...
for f in datafiles:
    ...
    dfg=pd.DataFrame(np.array([[np.datetime64(datetime.datetime(year,1,1)),np.datetime64(datetime.datetime(year,7,1))]]),index=[year])
    dfg_days=pd.DataFrame(np.array([[1,183]]),index=[pd.Timestamp(datetime.datetime(year,1,1))],columns=['Growing season start','Growing season end'])
    growing=False
    ...
    for index, row in df.iterrows():
        ...
        if yr<>year: #otherwise last year won't append
            ...
            dftemp_days=pd.DataFrame(np.array([[growing_start.dayofyear,growing_end.dayofyear]]),index=[pd.Timestamp(datetime.datetime(year,1,1))],columns=['Growing season start','Growing season end'])
